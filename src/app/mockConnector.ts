...
export type MockParameters = {
  ...
  features?:
    | {
        ...
        sendTransactionError?: boolean | Error | undefined;
      }
    | undefined;
};
    ...
    async getProvider({ chainId } = {}) {
      ...
      const request: EIP1193RequestFn = async ({ method, params }) => {
        ...
        if (method === "eth_sendTransaction") {
          if (features.sendTransactionError) {
            if (typeof features.sendTransactionError === "boolean")
              throw new UserRejectedRequestError(
                new Error("Failed to send transaction.")
              );
            throw features.sendTransactionError;
          }
        }
